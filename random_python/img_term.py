...
def img(input_img, height, width):
    ...
    for y in range(height // 2):
        ...
        for x in range(width):
            ...
            out.append(
                ''.join(("\x1B[38;5;", top_col, ";48;5;", bot_col, "m▀\x1B[0m")))
        out.append('\n')
    print("\x1b[;H")
    print(''.join(out))
...
def fast_setup():
    ...
    for idx, col in cols_bgr:
        ...
        lut[idx][1] = col[1]
        lut[idx][2] = col[2]
    g_lut = cl.Buffer(ctx, cl.mem_flags.READ_ONLY |
                      cl.mem_flags.COPY_HOST_PTR, hostbuf=lut)
    krnl = """
...
def img_fast(input_img, height, width, func, queue, ctx, g_lut):
    g_img = cl.Buffer(ctx, cl.mem_flags.READ_ONLY |
                      cl.mem_flags.COPY_HOST_PTR, hostbuf=input_img)
